...
| M03 | P0 | Timing split added; quant trial open | Improve the live translation path using M01 evidence: profile queue wait versus model decode, investigate capped/empty HY output, and test current Hy-MT2 Q2/Q3 against Q4 before recommending a quant. Keep bounded work and exact error reporting. Revisit the Easy setup local default if its translator remains too slow. | Captions do not accumulate a growing translation backlog; the default is supported by phone evidence and clearly distinguishes fast/light from higher-quality choices. No quant is labeled faster merely because its file is smaller. |
