...
Reviewed 2026-09-24 against the shipped model catalog, benchmark adapters,
...
| M06 | P1 | Pair presets and local export added; broader quality review open | The benchmark suggests pinned speed/balanced/quality trial pairs, queues supported downloads, runs installed speech and translation stages in one tap, and labels comparable saved speed/reference leaders. It accepts custom WAV or corrected text, retains 200 results, filters by language pair, and exports JSON. Finish names/numbers/negation, bilingual review and a live cascade run on the owner's S25. | A published table separates ASR error, translation adequacy, model load, stable source/translation delay, and memory. Six-sentence automatic scores are not presented as general quality rankings. |
