...
#include <jni.h>
namespace {
stt::concurrency::LeaseRegistry<hearth::ocr::PaddleOcr> models;
...
extern "C" JNIEXPORT jlong JNICALL Java_com_sal7one_common_1jni_ocr_PaddleNative_create(JNIEnv* e,jobject,jbyteArray d,jbyteArray r,jint classes){try{return models.insert(std::make_unique<hearth::ocr::PaddleOcr>(path(e,d),path(e,r),classes));}catch(const std::exception& x){fail(e,x.what());return 0;}catch(...){fail(e,"OCR initialization failed");return 0;}}
extern "C" JNIEXPORT jobjectArray JNICALL Java_com_sal7one_common_1jni_ocr_PaddleNative_recognize(JNIEnv* e,jobject,jlong id,jintArray argb,jint w,jint h,jint maxSide){
 try{
    ...
    auto lines=model->recognize(pixels.data(),w,h,maxSide);auto out=e->NewObjectArray(lines.size(),e->FindClass("[I"),nullptr);if(!out)return nullptr;
...
extern "C" JNIEXPORT void JNICALL Java_com_sal7one_common_1jni_ocr_PaddleNative_cancel(JNIEnv*,jobject,jlong id){auto model=models.acquire(id);if(model)model->cancel();}
extern "C" JNIEXPORT void JNICALL Java_com_sal7one_common_1jni_ocr_PaddleNative_destroy(JNIEnv*,jobject,jlong id){auto retired=models.retire(id);if(auto* m=retired.signalTarget())m->cancel();auto exclusive=std::move(retired).lockExclusive();}
