...
class CaptionLanguagesTest {
    ...
    @Test fun automaticAndFixedAdaptersDoNotClaimToPinSpeech() {
        val qwen = CaptionOverlayConfig(engine = CaptionEngineChoice.QWEN, streamLanguage = "ru")
        assertEquals(setOf("auto"), CaptionLanguages.source(qwen, SttMode.BATCH).codes)
        assertEquals("auto", CaptionLanguages.effectiveSource(qwen, SttMode.BATCH))
        assertEquals(SpeechProfile.QWEN3_ASR_0_6B.capabilities.sourceLanguages, CaptionLanguages.source(qwen, SttMode.BATCH).automaticLanguages)
