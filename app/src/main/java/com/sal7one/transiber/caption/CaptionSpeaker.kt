...
object CaptionSpeakerFactory {
    ...
    fun create(context: Context, choice: CaptionSpeakerChoice, onError: (String)->Unit = {}): CaptionSpeaker = when(choice) {
        ...
        else -> SharedCaptionVoice(context,when(choice){CaptionSpeakerChoice.SYSTEM->"system";CaptionSpeakerChoice.NATIVE->"supertonic";else->null},onError)
    }
}
private class SharedCaptionVoice(context: Context,backend: String?,error: (String)->Unit):CaptionSpeaker {
    private val player=VoicePlayer(context,backend){_,message->if(message!=null)error(message)}
    override fun speak(text: String,languageTag: String):Boolean {player.enqueue(text,languageTag);return text.isNotBlank()}
