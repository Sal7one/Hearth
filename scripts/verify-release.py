...
import hashlib, os, re, subprocess, tempfile, zipfile
...
expected_libs = {'libcommon_jni.so','libc++_shared.so','libvosk.so','libonnxruntime.so','libhearth_qwen.so','libhearth_nemotron.so','libandroidx.graphics.path.so','libdatastore_shared_counter.so'}
...
for flavor in ['play','foss']:
    ...
    print(f'{flavor}: {apk.stat().st_size} bytes; six speech libraries plus two AndroidX libraries; 16KB aligned; permissions PASS')
